...
from the dictionary that start with the input string. If there are more than 5 matches, restrict your
output to the first 5 results. If there are no matches, return an empty array.

Example:

autocomplete('ai', ['airplane','airport','apple','ball']) = ['airplane','airport']
For this kata, the dictionary will always be a valid array of strings. Please return all results in the
order given in the dictionary, even if they're not always alphabetical. The search should NOT be case
...
function autocomplete(input, dictionary) {
	let result = [];
	const correctedInput = input.replace(/[^a-z]/gi, '');

	if (correctedInput === '') return [];

	for (let word of dictionary) {
		if (word.toLowerCase().startsWith(correctedInput.toLowerCase())) {
			result.push(word);
		}
...
console.log(
	autocomplete('A12i*;', ['Airplane', 'airport', 'Ai', 'aipple', 'ai', 'aip'])
);
//['airplane','airport']
console.log(autocomplete('12*;', ['Airplane', 'airport', 'apple']));
//['airplane','airport']
