...
string -- it should behave in the same way even if the case of the minor word string is
changed.
...
function titleCase(title, minorWords) {
	...
	const wordsToCompare = minorWords
		.split(' ')
		.map((word) => word[0].toLowerCase() + word.slice(1).toLowerCase());
	const resultArr = words.map((word) => word.toLowerCase());
	for (let item of resultArr) {
		for (let elem of wordsToCompare) {
			if (item === elem) {
				finalArr.push(item);
			}
		}
	}
	const arr = resultArr.map((word) =>
		!finalArr.includes(word) ? word[0].toUpperCase() + word.slice(1) : word
	);
