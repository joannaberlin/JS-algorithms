...
to travel continuously around the arena; in discontiguous seating arrangements, the wave
can instead reflect back and forth through the crowd. When the gap in seating is narrow,
the wave can sometimes pass through it. Usually only one wave crest will be present at any
given time in an arena, although simultaneous, counter-rotating waves have been produced.
(Source Wikipedia)
...
function wave(str) {
	...
	for (let i = 0; i < arr.length; i++) {
		const char = arr[i].slice(i, i + 1).toUpperCase();
		const secondSlice = arr[i].slice(i + 1, arr.length);
		const firstSlice = arr[i].slice(0, i);
		const finalString = firstSlice + char + secondSlice;

		if (finalString.toLowerCase() !== finalString) {
